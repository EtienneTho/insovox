...
dataFolder = './out_01_acoustic_analysis/'
outFolder = './out_02_interpretations/'

#load data
data = pickle.load(open(str(dataFolder)+'withinCanonicalMaps.pkl', 'rb'))
allCanonicalMaps = data['allCanonicalMaps']         
training_accuracies_mean = data['training_accuracies_mean'] 
training_accuracies_std  = data['training_accuracies_std']  
testing_accuracies_mean  = data['testing_accuracies_mean']  
testing_accuracies_std   = data['testing_accuracies_std']   

print('training accuracies mean :')
print(training_accuracies_mean)
print('training accuracies std :')
print(testing_accuracies_std)
print('testing accuracies mean :')
print(training_accuracies_mean)
print('testing accuracies std :')
print(testing_accuracies_std)
print()
print()

# save interpretations as figures
nbChannels = 128
nbRates    = 14
nbScales   = 8
for iSubject in range(allCanonicalMaps.shape[0]):
  strf_session_mean_reshaped = np.reshape(allCanonicalMaps[iSubject],(nbChannels,nbScales,nbRates))    
  ...
  functions.plotStrfavg(strf_scale_rate, strf_freq_rate, strf_freq_scale,aspect_='auto', interpolation_='none',figname=outFolder+'subject#'+str(iSubject),show='false')
plt.show()
# compute PCA on interpretations
n_components = 2
pca = PCA(n_components=n_components, svd_solver='auto', whiten=True).fit(allCanonicalMaps)
allCanonicalMaps_pcs = pca.transform(np.asarray(allCanonicalMaps))

# plot interpretations
fig, ax = plt.subplots()
for i in range(allCanonicalMaps_pcs.shape[0]):
    print(i)
    print(allCanonicalMaps_pcs[i,0])
    # plt.scatter(allCanonicalMaps_pcs[i][0], allCanonicalMaps_pcs[i][1])    
    plt.text(allCanonicalMaps_pcs[i][0], allCanonicalMaps_pcs[i][1], str(i), fontsize=15)
...
#   allCanonicalMaps.append(np.mean(np.asarray(canonicalMapsFolds)))
