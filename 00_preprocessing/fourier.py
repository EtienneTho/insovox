...
import utils
# from features  #import spectrum2scaletime, scaletime2scalerate, scalerate2cortical, waveform2auditoryspectrogram
from spectrogram import compute_complex_spectrogram
...
def spectrogram(wavtemp,
    ...
    spectrogram_ = compute_complex_spectrogram(wavtemp, window_size,
                                               frame_step)
...
def spectrum(wavtemp,
             ...
             window_size=512,
             frame_step=128):
    spectrogram_ = np.abs(np.transpose(compute_complex_spectrogram(wavtemp, window_size, frame_step)))  
    
    spectrum_ = np.mean(spectrogram_[:,:window_size], axis=0)
    return spectrum_
